...
namespace netNS
{
    ...
    const int REPLY_SIZE = 12;
    const char CLIENT_ID[REPLY_SIZE]   = "Client v1.0";  // client ID
    const char SERVER_ID[REPLY_SIZE]   = "Server v1.0";  // server ID
    const char SERVER_FULL[REPLY_SIZE] = "Server Full";  // server full

    const int ERROR_CODES = 10;
    // Network error codes
    static const char *codes[ERROR_CODES] = {
        "No errors reported",
        "There is a General network error: ",
        "The Network init has failed: ",
        "This is an invalid socket: ",
        "The Get host by name has failed: ",
        "The Bind has failed: ",
        "The Connection has failed: ",
        "This Port is already in use: ",
        "The Domain has not been found: ",
        "This is an unknown network error: "
    };
...
private:
    ...
    int         ret;
    int         remoteAddrSize; //integeter to store the remote address size
    SOCKADDR_IN remoteAddr, localAddr;//socket address for both a remote address in the network and a local network IP address
    bool        netInitialized; //A booleaan to check if the network is intralized 
    bool        bound;
    char        mode;
    int         type;
    USHORT      port; //
...
public:
    ...
    Network();// Default Constructor
       virtual ~Network();// Default Destructor
